...
#' @param survdat the survival data with factors and interaction terms
#' expanded,  ordered by increasing survival time
...
#'     errors.}
#'
#'     \item{\code{haz}}{the estimated baseline hazard.  If \code{strat = TRUE}
#'     in the function call to \code{jointmeta1} then this is a list of length
#'     equal to the number of studies in the supplied dataset, each element of
#'     the list being the baseline hazard for the corresponding study. Otherwise
#'     there is a common baseline across all studies in the dataset and this is
#'     one vector.}
#'
#'     \item{\code{D}}{the estimated covariance matrix for the individual level
#'     random effects}
#'
#'     \item{\code{A}}{the estimated covariance matrix for the study level
#'     random effects.  This is only present in the output if study level random
#'     effects were specified in the function call to \code{jointmeta1}.}
#'
...
#'     estimated parameters of the joint model. The list is of length equal to
#'     the number of studies in the dataset, and each element of the list has
#'     number of rows equal to the number of individuals in the study, and
#'     number of columns equal to the number of specified individual level
#'     random effects.}
#'
...
#'     parameters of the joint model.  The matrix has number of rows equal to
#'     the number of studies, and number of columns equal to the number of
#'     specified study level random effects.}
#'
#'     \item{\code{long.rand.ind.form}}{a character string giving the
#'     formulation of the individual level random effects.}
#'
#'     \item{\code{long.rand.stud.form}}{a character string giving the
#'     formulation of the study level random effects if included in the model.}
#'
#'     \item{\code{conv}}{a logical value indicating whether convergence of the
#'     EM algorithm was achieved or not.}
#'
#'     \item{\code{iters}}{the number of iterations completed by the EM
#'     algorithm}
#'
#'     \item{\code{n.bystudy}}{the number of individuals present in each study
#'     in the data supplied to the function.}
#'     }
#'
...
EMalgRandprop <- function(data,longdat, survdat, long.rand.ind,
                          long.rand.stud = NULL, id.name, study.name,
                          gpt, max.it, tol, time.long, surv.formula,
                          long.formula, long.formula.orig, paraests, studies,
                          p1, p2, strat, print.detail, bootrun = FALSE,
                          q, r = NULL) {
  numstudies <- length(studies)
  ids.surv <- survdat[, 1]
  ids.bystudy <- lapply(1:numstudies, function(u) {ids.surv[
    which(survdat[, 4] == studies[u])]})
  ...
  tempdat<-merge(tempsurv,
                 longdat[
                   match(unique(longdat[,which(names(longdat) %in% id.name)]),
                         longdat[,which(names(longdat) %in% id.name)]),
                   c(1,5:ncol(longdat))],by=id.name)
  ...
  Z2.event.bystudy <- lapply(1:numstudies, function(u) {
    lapply(1:n.bystudy[[u]], function(v) {
      ...
      if(rstemp>0) {
        if(q>1) {
          temp <- matrix(rep(tZ2.bystudy[[u]][v, ], rstemp),
                         ncol = q, byrow = TRUE)
        }else{
          temp <- matrix(rep(tZ2.bystudy[[u]][v], rstemp),
                         ncol = q, byrow = TRUE)
  ...
  newu.2.all <- lapply(1:numstudies, function(u) {
    lapply(1:n.bystudy[u], function(v) {
      ...
      B2temp <- paraests$randstart.ind.cov[[
        which(names(paraests$randstart.ind.cov) %in% idtemp)]]
      eig <- eigen(B2temp)

      cm.2 <- matrix(as.numeric(as.character(
        paraests$randstart.ind[which(
          rownames(paraests$randstart.ind) %in% idtemp), ])), gpt^q, q, TRUE)
  ...
  randstart.ind.bystudy <- lapply(1:numstudies, function(u) {
    out <- as.matrix(randstart.ind[which(rownames(randstart.ind) %in%
                                           ids.bystudy[[u]]), ])
    rownames(out) <- rownames(randstart.ind)[
      which(rownames(randstart.ind) %in% ids.bystudy[[u]])]
    out
  })
  randstart.ind.bystudy <- lapply(1:numstudies, function(u) {
    randstart.ind.bystudy[[u]][match(ids.bystudy[[u]],
                                     rownames(randstart.ind.bystudy[[u]])), ]})
  if(is.null(long.rand.stud) == FALSE) {
    ...
    if(study.name %in% long.rand.stud) {
      ...
      long.rand.stud2 <- long.rand.stud
      long.rand.stud.form <- paste("-1", paste(long.rand.stud2,
                                               collapse = "+"), sep = "+")
    }
    ...
    Z3Z2.event.sqr.bystudy <- lapply(1:numstudies, function(u) {
      lapply(1:n.bystudy[u], function(v) {
        ...
        if(rstemp>0) {
          Z3.event.bystudy[[u]][[v]][randselector1, ] *
            Z2.event.bystudy[[u]][[v]][randselector2, ]
        }else{
    ...
    newu.3.all <- lapply(1:numstudies, function(u) {
      studtemp <- studies[u]
      B3temp <- paraests$randstart.stud.cov[[
        which(names(paraests$randstart.stud.cov) %in% studtemp)]]
      eig <- eigen(B3temp)
      cm.3 <- matrix(as.numeric(as.character(
        paraests$randstart.stud[which(rownames(paraests$randstart.stud) %in%
    ...
    survnames<-NULL
    if(p2 > 0) {
      survnames<-paste("T.", names(beta2)[1:(length(beta2)-2)], sep = "")
    }
    paranames <- c(paste("Y.", names(beta1), sep = ""),
                   survnames,
                   "gamma_ind_0",
                   "gamma_stud_0",
                   "sigma.e",
                   paste("D", paste( rep(1:q, times = q),
                                     rep(1:q, each = q), sep = ""), sep = ""),
                   paste("A", paste( rep(1:r, times = r),
                                     rep(1:r, each = r), sep = ""), sep = ""))
    }else{
      r <- NULL
      survnames<-NULL
      if(p2 > 0) {
        survnames<-paste("T.", names(beta2)[1:(length(beta2)-1)], sep = "")
      }
      paranames <- c(paste("Y.", names(beta1), sep = ""),
                     survnames,
                     "gamma_ind_0",
  ...
  for (it in 1:max.it) {
    ...
    ebeta2x <- lapply(1:numstudies, function(u) {exp(beta2x[[u]])})
    for(k in 1:numstudies) {
      if(is.null(long.rand.stud) == FALSE) {
        ...
        newu.3sqr <- newu.3^2
        if(r>1) {
          newu.3sqr <- cbind(newu.3sqr, newu.3[, s1.3] * newu.3[, s2.3])
        }
        egDUs.3 <- t(apply(exp(newu.3 %*% (Z3.surv.bystudy[[k]] *
                                             beta2[(p2 + 2)])), 1,
                           function(u) {u^cen.bystudy[[k]]}))
        egDUsf.3 <- lapply(1:n.bystudy[k], function(v) {
          exp(newu.3 %*% (Z3.event.bystudy[[k]][[v]] * beta2[(p2 + 2)]))
        })
        if(q>1) {
        ...
        ess.3 <- lapply(1:n.bystudy[k], function(v) {
          if(strat) {
            exp(-(ebeta2x[[k]][v, ] * egDUsf.3[[v]] ) %*%
                  ((haz[[k]][1:rs[[k]][which(names(rs[[k]]) %in%
                                               ids.bystudy[[k]][[v]])]] *
                      as.numeric(egDUsf.2[[v]]))))
          }else{
            exp(-(ebeta2x[[k]][v, ] * egDUsf.3[[v]] ) %*%
                  ((haz[1:rs[which(names(rs) %in% ids.bystudy[[k]][[v]])]] *
                      as.numeric(egDUsf.2[[v]]))))
          }
        })
      ...
      for (i in 1:n.bystudy[k]) {
        ...
        newu.2sqr <- newu.2^2
        if(q>1) {
          newu.2sqr <- cbind(newu.2sqr, newu.2[, s1.2] * newu.2[, s2.2])
        }
        egDUs.2.ind <- 1
        egDUs.3.const <- 1
        egDUs.3.ind <- 1
        ...
        if(is.null(long.rand.stud) == FALSE) {
          ...
          Z3b3event[[k]][[i]][1:ncol(Z3.event.bystudy[[k]][[i]]), ] <-
            (t(Z3.event.bystudy[[k]][[i]] *
                 as.numeric(f.3.ind[, 1] %*%newu.3))) / den.3.ind
          Z3b3eventsqr[[k]][[i]][1:ncol(Z3.event.sqr.bystudy[[k]][[i]]), ] <-
            (as.numeric(f.3.ind[, 1] %*%newu.3sqr) *
               Z3.event.sqr.bystudy[[k]][[i]]) / den.3.ind
        }
        if(strat) {
          if(is.null(long.rand.stud) == FALSE) {
        ...
        Z2b2event[[k]][[i]][1:ncol(Z2.event.bystudy[[k]][[i]]), ] <-
          (t(Z2.event.bystudy[[k]][[i]] *
               as.numeric(f.2[, 1] %*%newu.2))) / den.2
        Z2b2eventsqr[[k]][[i]][1:ncol(Z2.event.sqr.bystudy[[k]][[i]]), ] <-
          (as.numeric(f.2[, 1] %*%newu.2sqr) *
             Z2.event.sqr.bystudy[[k]][[i]]) / den.2
      }
    }
    ...
    if(strat == FALSE) {
      haz <- nev / colSums(do.call(rbind, EexpU) * do.call(rbind, ebeta2x)[, 1])
    }else{
      haz <- lapply(1:numstudies, function(u) {nev[[u]] /
          colSums(EexpU[[u]] * ebeta2x[[u]][, 1])})
    }
    EUmat.2 <- lapply(1:numstudies, function(u) {apply(EU.2[[u]], 2,
                                                       rep, nn[[u]])})
    EUUmat.2 <- lapply(1:numstudies, function(u) {apply(EUU.2[[u]], 2,
                                                        rep, nn[[u]])})
    Ut.2 <- lapply(1:numstudies, function(u) {rowSums(EUmat.2[[u]] *
                                                        tZ2.bystudy[[u]])})
    if(is.null(long.rand.stud) == FALSE) {
      ...
      beta1 <- as.numeric(solve(crossprod(do.call(rbind, X1.bystudy)),
                                crossprod(do.call(rbind, X1.bystudy),
                                          do.call(c, Y.bystudy) -
                                            (do.call(c, Ut.2) +
                                               do.call(c, Ut.3)))))
      if(q == 1) {
        if(r == 1) {
          sigma.e <- colSums((do.call(c, Y.bystudy)-
                                ((do.call(rbind, X1.bystudy) %*%beta1) +
                                   (do.call(c, Z2.bystudy) *
                                      do.call(c, lapply(1:numstudies,
                                                        function(u) {
                                                          rep(EU.2[[u]],
                                                              nn[[u]])
                                                        }))) +
                                   (do.call(c, Z3.bystudy) *
                                      do.call(c,
                                              lapply(1:numstudies,
                                                     function(u) {
                                                       EU.3[rep(
      ...
      if(q == 1) {
        ...
        temp1<-do.call(rbind,lapply(1:numstudies,
                                    function(u) {
                                      EU.2[[u]][rep(
                                        1:nrow(EU.2[[u]]),
                                        nn[[u]]), ]}))
    ...
    if(q>1) {
      fd[(p2 + 1):(p2 + q)] <- colSums(do.call(c, cen.bystudy) *
                                         (do.call(rbind, EU.2) *
                                            do.call(rbind,
                                                    tZ2.surv.bystudy))) -
        colSums(do.call(rbind, ebeta2x)[, 1] *
                  do.call(rbind, lapply(1:numstudies, function(u) {
                    if(strat) {
                      haztemp <- haz[[u]]
                    }else{
                      haztemp <- haz
      ...
      fd[(p2 + 1):(p2 + q)] <- sum(do.call(c, cen.bystudy) *
                                     (do.call(rbind, EU.2) *
                                        do.call(c, tZ2.surv.bystudy))) -
        colSums(do.call(rbind, ebeta2x)[, 1] *
                  do.call(rbind, lapply(1:numstudies, function(u) {
                    if(strat) {
                      haztemp <- haz[[u]]
    ...
    if(q>1) {
      if(q == 2) {
        sd[(p2 + 1):(p2+q), (p2 + 1):(p2 + q)][
          upper.tri(sd[(p2 + 1):(p2 + q), (p2 + 1):(p2 + q)])] <-
          -sum(do.call(rbind, ebeta2x)[, 1] * 0.5 *
                 do.call(rbind, lapply(1:numstudies, function(u) {
                   if(strat) {
                     haztemp <- haz[[u]]
                   }else{
        ...
        sd[(p2 + 1):(p2 + q), (p2 + 1):(p2 + q)][
          upper.tri(sd[(p2 + 1):(p2 + q), (p2 + 1):(p2 + q)])] <-
          -colSums(do.call(rbind, ebeta2x)[, 1] * 0.5 *
                     do.call(rbind, lapply(1:numstudies, function(u) {
                       if(strat) {
                         haztemp <- haz[[u]]
                       }else{
    ...
    if(is.null(long.rand.stud) == FALSE) {
      if(r>1) {
        if(r == 2) {
          sd[(p2 + q + 1):(p2 + q + r), (p2 + q + 1):(p2 + q + r)][
            upper.tri(sd[(p2 + q + 1):(p2+q+r), (p2 + q + 1):(p2+q+r)])] <-
            -sum(do.call(rbind, ebeta2x)[, 1] * 0.5 *
                   do.call(rbind, lapply(1:numstudies, function(u) {
                     if(strat) {
                       haztemp <- haz[[u]]
        ...
        sd[(p2 + 1):(p2 + q), ((p2 + q + 1):(p2 + q + r)) ] <-
          -colSums(do.call(rbind, ebeta2x)[, 1] *
                     do.call(rbind, lapply(1:numstudies, function(u) {
                       ...
                       do.call(rbind, lapply(1:n.bystudy[u], function(v) {
                         sum(Z3b3event[[u]][[v]] * Z2b2event[[u]][[v]] *
                               EexpU.2[[u]][v, ] * EexpU.3[[u]][v, ] * haztemp)
                       }))
                     })))
      }else{
        sd[(p2 + 1):(p2 + q), ((p2 + q + 1):(p2 + q + r)) ] <-
          -colSums(do.call(rbind, ebeta2x)[, 1] *
                     do.call(rbind, lapply(1:numstudies, function(u) {
                       ...
                       do.call(rbind, lapply(1:n.bystudy[u], function(v) {
                         colSums(Z3b3event[[u]][[v]][, randselector1] *
                                   Z2b2event[[u]][[v]][, randselector2] *
                                   EexpU[[u]][[v]] * haztemp)
                       }))
                     })))
      }
    ...
    if(p2>0) {
      fd[1:p2] <- c(colSums((do.call(c, cen.bystudy) *
                               do.call(rbind, X2.bystudy)) -
                              (do.call(rbind, X2.bystudy) *
                                 as.numeric(
                                   do.call(rbind, ebeta2x)[, 1] *
                                     do.call(rbind,
                                             lapply(1:numstudies,
                                                    function(u) {
                                                      if(strat) {
                                                      ...
                                                      lapply(1:numstudies,
                                                             function(u) {
                                                               ...
                                                               do.call(rbind,
                                                                       lapply(1:n.bystudy[u],
                                                                              function(v) {
                                                                                colSums(Z2b2event[[u]][[v]] *
                                                                                          EexpU[[u]][[v]] *
                                                                                          haztemp)
                                                                              }))
                                                             }))))
      if(is.null(long.rand.stud) == FALSE) {
        sd[(1:p2), (p2 + q + 1):(p2 + q + r)] <-
          -(t(do.call(rbind, X2.bystudy)) %*%
              (do.call(rbind, ebeta2x)[, 1] *
                 do.call(rbind, lapply(1:numstudies, function(u) {
                   if(strat) {
                     haztemp <- haz[[u]]
      ...
      for (i in 1:p2) {
        for (j in 1:p2) {
          sd[i, j] <- -(sum(do.call(rbind, X2.bystudy)[, i] *
                              do.call(rbind, X2.bystudy)[, j] *
                              do.call(rbind, ebeta2x)[, 1] *
                              do.call(rbind, lapply(1:numstudies, function(u) {
                                if(strat) {
                                  haztemp <- haz[[u]]
                                ...
                                do.call(rbind, lapply(1:n.bystudy[u],
                                                      function(v) {
    ...
    if(q == 1) {
      sd[(p2 + 1), (p2 + 1)] <- -(colSums(do.call(rbind, ebeta2x)[, 1] *
                                            do.call(rbind,
                                                    lapply(1:numstudies,
                                                           function(u) {
                                                             if(strat) {
                                                             ...
                                                             do.call(rbind,
                                                                     lapply(1:n.bystudy[u],
                                                                            function(v) {
                                                                              colSums(Z2b2eventsqr[[u]][[v]] *
                                                                                        EexpU[[u]][[v]] *
                                                                                        haztemp)
                                                                            }))
                                                           }))))[1:q]
    }else{
      diag(sd[(p2 + 1):(p2 + q), (p2 + 1):(p2 + q)]) <-
        -(colSums(do.call(rbind, ebeta2x)[, 1] *
                    do.call(rbind, lapply(1:numstudies, function(u) {
                      if(strat) {
                        haztemp <- haz[[u]]
                      ...
                      do.call(rbind, lapply(1:n.bystudy[u], function(v) {
                        colSums(Z2b2eventsqr[[u]][[v]] * EexpU[[u]][[v]] *
                                  haztemp)
                      }))
                    }))))[1:q]
    }
    if(is.null(long.rand.stud) == FALSE) {
      if(r == 1) {
        ...
        diag(sd[(p2 + q + 1):(p2 + q + r), (p2 + q + 1):(p2 + q + r)]) <-
          -(colSums(do.call(rbind, ebeta2x)[, 1] *
                      do.call(rbind, lapply(1:numstudies, function(u) {
                        if(strat) {
                          haztemp <- haz[[u]]
      ...
      if(r == 1 && q == 1) {
        ...
        if(p2>0){
          fd <- c(fd[1:p2], sum(fd[(p2 + 1):(p2 + q)]),
                  sum(fd[(p2 + q + 1):(p2 + q + r)]))
          sd.p2 <- sd[1:p2, 1:p2]
          sd.q <- sum(sd[(p2 + 1):(p2 + q), (p2 + 1):(p2 + q)])
          sd.r <- sum(sd[(p2 + q + 1):(p2 + q + r), (p2 + q + 1):(p2 + q + r)])
          if(p2>1) {
            if(q > 1){
            ...
            if(r > 1){
              sd.p2r <- rowSums(sd[1:p2, (p2 + q + 1):(p2 + q + r)])
            } else{
          ...
          sd.qr <- sum(sd[(p2 + 1):(p2 + q), (p2 + q + 1):(p2 + q + r)])
          sd <- rbind(cbind(sd.p2, sd.p2q, sd.p2r),
                      c(sd.p2q, sd.q, sd.qr), c(sd.p2r, sd.qr, sd.r))
  ...
  results <- list(beta1 = data.frame(beta1), beta2 = data.frame(beta2),
                  sigma.e = sigma.e, D = D, haz = haz, random2 = EU.2,
                  conv = conv, iters = it,
                  long.rand.ind.form = long.rand.ind.form,n.bystudy =
                    n.bystudy)
  if(is.null(long.rand.stud) == FALSE) {
    results$A <- A
