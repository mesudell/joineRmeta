...
#'   components included in \code{fitted} (see \code{\link{jointmeta1.object}}).
#'
#' @export
#'
#' @examples
...
print.jointmeta1 <- function(fitted) {
  ...
  cat("\n")
  cat("Longitudinal sub-model fixed effects:", deparse(fitted$Call$long.formula))
  longfixed <- fitted$coefficients$fixed$longitudinal
  colnames(longfixed) <- ""
  print(longfixed)
  cat("\n")
  cat("Time-to-event sub-model fixed effects:", deparse(fitted$Call$surv.formula))
  cat("\n")
  ...
  if (is.null(fitted$rand_cov$A) == FALSE) {
    ...
    paranames <- c(rownames(fitted$rand_cov$D), rownames(fitted$rand_cov$A),
                   "")
