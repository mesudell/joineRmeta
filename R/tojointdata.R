...
#'     long or wide format holding both the longitudinal and survival
#'     information (and any baseline information).  Either the parameter
#'     \code{dataset} should be specified, or the parameters \code{longitudinal}
#'     and \code{survival} (and \code{baseline} if available) should be
#'     supplied.
...
#'     censoring variable).  It can also contain baseline information. This
#'     should not be supplied if \code{dataset} is supplied to the function
#'     call.
...
#'     studies may be supplied.  This are discussed below.  We refer to wide
#'     data as data that contains both time varying and time stationary data,
#'     with one line per individual with variables measured over time recorded
#'     in multiple columns.  We refer to long data as data with multiple lines
#'     per individual, with time varying covariates differing between rows, but
#'     time stationary covariates identical between rows.  Survival data are
#'     considered data with survival outcome (survival time and censoring
#'     variable) with or without baseline data.  Longitiudinal datasets are
#'     considered long format datasets containing time varying data potentially
#'     with baseline data. Baseline datasets are considered wide format datasets
#'     containing non-time varying data measured at baseline.
#'     This function can take data in the following formats and output a
#'     \code{jointdata} object. \describe{
#'
...
#'     \code{NULL}, or left unspecified in the function call.  This dataset
#'     would contain all the data from all studies, and the survival,
#'     longitudinal and any baseline information would all be present in the
#'     same dataset.}
#'
...
#'     function call with \code{longitudinal}, \code{survival} and
#'     \code{baseline} set to \code{NULL}, or left unspecified in the function
#'     call.  This dataset  would contain all the data from all studies, and the
#'     survival, longitudinal and any baseline information would all be present
#'     in the same dataset.}
#'
...
#'     \code{survival} and \code{baseline} set to \code{NULL}, or left
#'     unspecified in the function call.  The data from each study would contain
#'     the survival, longitudinal and any baseline information.}
#'
#'     \item{\code{A list of study specific long datasets}}{One dataset for each
#'     study, each in long format (multiple rows per individual), supplied to
#'     the parameter \code{dataset} in the function call with
#'     \code{longitudinal}, \code{survival} and \code{baseline} set to
#'     \code{NULL} or left unspecified in the function call.  The data from each
#'     study would contain the survival, longitudinal and any baseline
#'     information.}
#'
#'     \item{\code{One longitudinal and one survival dataset with or without
#'     an additional baseline dataset}}{In this case all the longitudinal and
#'     time varying data for all studies is supplied in a single dataset in long
#'     format to the parameter \code{longitudinal}.  All the survival data for
#'     all studies is supplied in a single dataset in wide format to the
#'     parameter \code{survival}.  Baseline data can be present in these two
#'     datasets, or can also be supplied as a dataset to the parameter
#'     \code{baseline}. If \code{longitudinal} and \code{survival} are
#'     specified, then parameter \code{dataset} should be
#'     set to \code{NULL} or left unspecified in the function call.
#'     The parameter \code{baseline} is optional, but
#'     should only be specified if parameter \code{dataset} is \code{NULL} or
#'     unspecified.}
#'
#'     \item{\code{A list of longitudinal and a list of survival datasets with
#'     or without a list of baseline datasets}}{In this case the
#'     longitudinal and time varying data for each study is supplied as one
#'     element of a list of long format datasets to the parameter
#'     \code{longitudinal}.  The survival data for each study is supplied as
#'     one element of a list of wide format datasets to the parameter
#'     \code{survival}. Baseline data can be present in these two sets of
#'     datasets, or can be supplied as an additional list of datasets one for
#'     each study to the parameter \code{baseline}. If \code{longitudinal} and
#'     \code{survival} are specified (\code{baseline} is optional), then
#'     parameter \code{dataset} should be set to \code{NULL}, or left
#'     unspecified in the function call.}
#'
#'     }
#'
#'     The specified id variable should be present in all datasets supplied to
#'     the function.  Variables containing the same information should be
#'     identically named in each supplied dataset, for example if a variable
#'     \code{"age"} is present in one dataset, denoting age of individual at
#'     baseline, corresponding variables in other datasets also supplying age
#'     at baseline should also be named \code{"age"}.  Similarly, different
#'     variables should not share the same name across different datasets, for
#'     example there should not be a variable named \code{"age"} in the
#'     longitudinal dataset denoting individual's age at last longitudinal
#'     measurement along with a variable \code{"age"} in the baseline dataset
#'     that denotes age of the individual at baseline.  Before supplying data
#'     to this function, names of variables in each dataset should be checked
#'     to confirm that common variables share the same name, and differing
#'     variables are appropriately distinguished from each other.
#'
#' @export
#' @import joineR
#'
...
tojointdata <- function(dataset = NULL, longitudinal = NULL, survival = NULL,
                      baseline = NULL, id, longoutcome, timevarying = NULL,
                      survtime, cens, time = NULL, longtimes = NULL){
  if(missing(id)){
  ...
  if(is.null(dataset) == FALSE){
    ...
    if(datatype == "wide"){
      ...
      survivaltemp <- survivaltemp[, c(which(names(survivaltemp)%in%id),
                                      which(names(survivaltemp)%in%survtime),
                                      which(names(survivaltemp)%in%cens))]
      if(is.null(timevarying) == FALSE){
        timevariable <- lapply(c(longoutcome, timevarying),
                               function(u){
                                 names(dataset)[grepl(u, names(dataset))]
                                 })
      }else{
        timevariable <- lapply(longoutcome,
                               function(u){
                                 names(dataset)[grepl(u, names(dataset))]
                                 })
      }
      ...
      longtemp <- reshape(dataset, varying = timevariable,
                          v.names = c(longoutcome, timevarying),
                          times = longtimes, direction = "long")
      if(is.null(timevarying)){
        longtemp <- longtemp[,
                             which(names(longtemp)%in%c(id,
                                                        longoutcome,
                                                        "time"))]
      }else{
        longtemp <- longtemp[,
      ...
      if(length(which(!(names(longtemp)%in%c(id, longoutcome, time)))) > 0){
        longtemp <- longtemp[, c(which(names(longtemp)%in%id),
                                which(names(longtemp)%in%longoutcome),
                                which(names(longtemp)%in%time),
                                which(!(names(longtemp)%in%c(id,
                                                             longoutcome,
                                                             time))))]
      }else{
      ...
      if(length(rows[is.na(survivaltemp[,2])]) > 0){
        survivalremove<-c(survivalremove,rows[is.na(survivaltemp[,2])])
      }
      if(length(rows[is.na(survivaltemp[,3])]) > 0){
        survivalremove<-c(survivalremove,rows[is.na(survivaltemp[,3])])
      }
      ...
      longtemp <- dataset[, which(names(dataset)%in%c(id,
                                                     longoutcome,
                                                     timevarying))]
      if(length(which(!(names(longtemp)%in%c(id, longoutcome, time)))) > 0){
        longtemp <- longtemp[, c(which(names(longtemp)%in%id),
      ...
      rownames(survivaltemp) <- 1:nrow(survivaltemp)
      survivaltemp <- dataset[match(unique(dataset[, idcol]),
                                    dataset[, idcol]),
                              which(names(dataset)%in%c(id, survtime, cens))]
      survivaltemp <- survivaltemp[, c(which(names(survivaltemp)%in%id),
                                      which(names(survivaltemp)%in%survtime),
                                      which(names(survivaltemp)%in%cens))]
      baselinecols <- which(!(names(dataset)%in%c(names(survivaltemp),
                                                  names(longtemp))))
      if(length(baselinecols) > 0){
        baselinetemp <- dataset[match(unique(dataset[, idcol]), dataset[,
                                                                        idcol]),
                                c(idcol, baselinecols)]
      }else{
        baselinetemp <- NULL
      }
    }
    ...
    baselinetemp.long <- longitudinal[match(unique(longitudinal[, idcol.long]),
                                            longitudinal[, idcol.long]),
                                      which(!(names(longitudinal)%in%c(
                                        longoutcome, timevarying)))]
    survivaltemp <- survival[, which(names(survival)%in%c(id, survtime, cens))]
    survivaltemp <- survivaltemp[, c(which(names(survivaltemp)%in%id),
                                    which(names(survivaltemp)%in%survtime),
                                    which(names(survivaltemp)%in%cens))]
    rows<-1:nrow(survivaltemp)
    ...
    if(is.null(baseline) == FALSE){
      if(class(baseline) == "list"){
        baseline <- Reduce(function(x, y) merge(x, y, all = TRUE), baseline)
      }
  ...
  survivaltemp <- survivaltemp[order(survivaltemp[,
                                                  which(names(survivaltemp)%in%
                                                          id)]), ]
  baselinetemp <- baselinetemp[order(baselinetemp[,
                                                  which(names(baselinetemp)%in%
                                                          id)]), ]
