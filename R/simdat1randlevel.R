...
simdat1randlevel <- function(k, n, rand_ind, sepassoc, ntms, longmeasuretimes,
                             beta1, beta2, gamma, sigb_ind, vare, theta0,
                             theta1, censoring, censlam, truncation,
                             trunctime, q) {
  ...
  Y <- lapply(1:k, function(u) {
    X1[[u]] %*% beta1 + colSums(Z2b2[[u]]) + sqrt(vare) * rnorm(n[u] * ntms)
    })
  ...
  if(rand_ind == "int") {
    if(class(gamma) == "list") {
      ...
      survtime <- lapply(1:k, function(u) {
        -log(uu[[u]])/exp(theta0[[u]] + beta2x[[u]] + (U_ind[[u]] * gamma))
      })
    }
  }else{
    if(class(gamma) == "list") {
      ...
      survtime <- lapply(1:k, function(u) {
        ...
        temp2 <- exp(theta0[[u]] + beta2x[[u]] + (gamma[1] * U_ind[[u]][, 1]))
        ii <- (temp1<0)&(uu[[u]]<exp(temp2/temp1))
        survtime <- rep(0, n[u])
        survtime[ii] <- Inf
        survtime[!ii] <- (1/temp1[!ii]) *
