...
#' function call
#' @param longdat2 the longitudinal dataset, without factors expanded
#' into dummy variables, ordered by increasing survival time
#' @inheritParams jointmeta1
#' @inheritParams EMalgRandprop
#'
#'
#' @return A list of results from the separate longitudinal fit is returned.
#'     The elements of this list are: \describe{
#'
...
#'     longitudinal model}
#'
#'     \item{\code{randstart.ind}}{the conditional modes of the individual
#'     level random effects given the data and the parameter estimates from the
#'     separate longitudinal model. The data frame has number of columns equal
#'     to the number of individual level random effects, and the number of rows
#'     equal to the number of individuals in the dataset}
#'
...
#'     The list is of length equal to the number of individuals in the dataset.
#'     Each element of the list is a matrix of dimensions equal to the number
#'     of study level random effects in the model.}
#'
#'     \item{\code{A}}{a data frame containing the estimate of the covariance
#'     matrix for the study level random effects.  This is present in the
#'     results only if study level random effects are included in the model.}
#'
#'     \item{\code{randstart.stud}}{the conditional modes of the study level
#'     random effects given the data and the parameter estimates from the
#'     separate longitudinal model.  The data frame has number of columns equal
#'     to the number of study level random effects, and number of rows equal to
#'     the number of studies in the dataset.  This is present in the results
...
#'     matrices for each study for the study level random effects.
#'     The list is of length equal to the number of studies in the dataset.
#'     Each element of the list is a matrix of dimensions equal to the number
#'     of study level random effects in the model.  This is present in the
#'     results only if study level random effects are included in the model.}
#'
#'     \item{\code{modelfit}}{the initial longitudinal model fit, fitted using
#'     the \code{\link[lme4]{lmer}} function from the \code{\link[lme4]{lme4}}
#'     package.}
#'
#'     }
#' @keywords internal
#' @import lme4
longst <- function(longdat, long.formula.orig, long.rand.ind,
                   long.rand.stud = NULL, longdat2, id.name,
                   study.name, studies) {
  if("(Intercept)"%in%long.rand.ind) {
    remainingrandind <- long.rand.ind[-which((long.rand.ind%in%
                                                "(Intercept)")==TRUE)]
    if(length(remainingrandind) > 0) {
      remainingrandind <- unlist(lapply(1:length(remainingrandind),function(u){
        names(longdat[5:ncol(longdat)])[
          match(remainingrandind[u],names(longdat[5:ncol(longdat)]))]
      }))
      rf <- paste("(", paste("1",
                             paste(remainingrandind,
                                   collapse = " + "),
                             sep = " + "), "|", id.name, ")",
                  collapse = "", sep = "")
    }else{
  ...
  if("noint"%in%long.rand.ind) {
    remainingrandind <- long.rand.ind[-which((long.rand.ind%in%"noint")==TRUE)]
    if(length(remainingrandind) > 0) {
      remainingrandind <- names(longdat[5:ncol(longdat)])[
        which(names(longdat[5:ncol(longdat)])%in%remainingrandind)]
      rf <- paste("(", paste("-1",
                             paste(remainingrandind,
                                   collapse = " + "),
                             sep = " + "), "|", id.name, ")",
  ...
  if(is.null(long.rand.stud)==FALSE) {
    if(study.name%in%long.rand.stud) {
      ...
      if(length(remainingrandstudy) > 0) {
        remainingrandstudy <- names(longdat[5:ncol(longdat)])[
          which(names(longdat[5:ncol(longdat)])%in%remainingrandstudy)]
        rf <- paste(rf, " + (", paste("1",
                                      paste(remainingrandstudy,
                                            collapse = " + "),
                                      sep = " + "), "|", study.name, ")",
                    collapse = "", sep = "")
      }
      if(length(remainingrandstudy)==0) {
        rf <- paste(rf, " + (1|", study.name, ")", collapse = "", sep = "")
      }
    ...
    if((study.name%in%long.rand.stud)==FALSE) {
      ...
      for(i in 1:length(long.rand.stud)) {
        cols<-c(cols,
                grep(long.rand.stud[i], names(longdat[5:ncol(longdat)])))
      }
      long.rand.stud2 <- names(longdat[5:ncol(longdat)])[cols]
      rf <- paste(rf, " + (", paste("-1",
                                    paste(long.rand.stud2,
                                          collapse = " + "),
                                    sep = " + "), "|", study.name, ")",
                  collapse = "", sep = "")
  ...
  long.start <- lmer(as.formula(paste(long.formula.orig2[2], "~",
                                      long.formula.orig2[3], " + ", rf,
                                      collapse = "")),
                     data = data.frame(longdat), na.action = na.omit)
  randout <- ranef(long.start, condVar = TRUE)
  if(is.null(long.rand.stud)==FALSE) {
    r <- num.rand.study
    if(r > 1) {
      A <- as.data.frame(VarCorr(long.start)[ which(
        names(VarCorr(long.start))%in%study.name)])
    }
    if(r==1) {
      A <- as.data.frame(VarCorr(long.start)[ which(
        names(VarCorr(long.start))%in%study.name)])
    ...
    if((study.name%in%long.rand.stud)==TRUE) {
      remainingrandstudy <- long.rand.stud[-which((
        long.rand.stud%in%study.name)==TRUE)]
      if(length(remainingrandstudy) > 0) {
  ...
  randstart.ind.cov <- lapply(1:length(unique(longdat[[id.name]])),
                              function(u) {
                                attr(randout[[id.name]], "postVar")[1:q, 1:q, u]
                              })
  names(randstart.ind.cov) <- rownames(randstart.ind)
  if(q > 1) {
    D <- as.data.frame(VarCorr(long.start)[ which(
      names(VarCorr(long.start))%in%id.name)])
    if("noint"%in%long.rand.ind) {
      long.rand.ind.temp <- long.rand.ind[-which(long.rand.ind%in%"noint")]
    }else{
    ...
    D <- as.data.frame(VarCorr(long.start)[
      which(names(VarCorr(long.start))%in%id.name)])
    if("noint"%in%long.rand.ind) {
      long.rand.ind.temp <- long.rand.ind[-which(long.rand.ind%in%"noint")]
    }else{
      long.rand.ind.temp <- long.rand.ind
  ...
  sigma.e <- as.numeric(as.data.frame(VarCorr(long.start))[
    nrow(as.data.frame(VarCorr(long.start))), 5])^2
  ll <- as.numeric(logLik(long.start))
  beta1 <- fixef(long.start)
  out <- list(beta1 = data.frame(beta1), sigma.e = sigma.e, D = D,
            log.like.long = ll, randstart.ind = randstart.ind,
            randstart.ind.cov = randstart.ind.cov,
            modelfit = long.start)
  if(is.null(long.rand.stud)==FALSE)
  {
    out$A <- A
