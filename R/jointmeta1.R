...
#'
#' Function to allow a one stage joint model (data from all studies analysed
#'     in one model)  to be fitted to data from multiple studies.  The function
#'     allows one longitudinal and one time-to-event outcome, and can
#'     accommodate baseline hazard stratified or not stratified
#'     by study, as well as random effects at the individual level and the study
#'     level.  Currently only zero mean random effects only proportional
#'     association supported - see Wulfsohn and Tsiatis 1997
#'
...
#' @param long.rand.ind a vector of character strings to indicate what
#'     variables to assign individual level random effects to.  A maximum of
#'     three individual level random effects can be assigned.  To assign a
#'     random intercept include "int" in the vector.  To not include an
#'     individual level random intercept include "noint" in the vector.  For
#'     example to fit a model with individual level random intercept and random
#'     slope set \code{long.rand.ind = c("int", "time")}, where
#'     \code{"time"} is the longitudinal time variable in the \code{data}.
...
#'     parameters are calculated for the linear combination of random effects
#'     at each level.  There are plans to expand to more sharing structures in
#'     the future.
...
#'     fit and the log-likelihood from a separate linear mixed model analysis
#'     of the longitudinal data are returned (see the \code{\link[lme4]{lmer}}
#'     function).  The separate longitudinal model fit has the same
#'     specification as the longitudinal sub-model of the joint model.
#' @param survsep logical value: if \code{TRUE} then parameter estimates, model
#'     fit and log-likelihood from a separate analysis of the survival data
#'     using the Cox Proportional Hazards model are returned (see
...
#'     to survival and longitudinal data from multiple studies. This model is
#'     an extension of the model proposed by Wulfsohn and Tsiatis (1997).  The
#'     model must contain at least one individual level random effect (specified
#'     using the \code{long.rand.ind} argument).  The model can also contain
#'     study level random effects (specified using the  \code{long.rand.stud}
#'     argument), which can differ from the individual level random effects.
#'     The maximum number of random effects that can be specified at each level
#'     is three. Note that the fitting and bootstrapping time increases as the
#'     number of included random effects increases.  The model can also include
#'     a baseline hazard stratified by study, or can utilise a common baseline
#'     across the studies in the dataset.  Interaction terms can be specified in
#'     either the longitudinal or the survival sub-model.
#'
...
#'     \eqn{X_1} represents the design matrix for the longitudinal fixed
#'     effects.  The subscript 1 is used to distinguish between items from the
#'     longitudinal sub-model and items from the survival sub-model (which
#'     contain a subscript 2).  The design matrices for random effects are
#'     represented using \eqn{Z}, fixed effect coefficients are represented by
#'     \eqn{\beta}, random effects by \eqn{b} and the measurement error by
#'     \eqn{\epsilon}.  Study membership is represented by the subscript \eqn{k}
#'     whilst individuals are identified by \eqn{i} and time points at which
#'     they are measured by \eqn{j}.  The longitudinal outcome is assumed
#'     continuous.
#'
#'     Currently this function only supports one linking structure between the
#'     sub-models, namely a random effects only proportional sharing structure.
#'     In this structure, the zero mean random effects from the longitudinal
...
#'     survival sub-model (for a case without baseline stratified by study)
...
#'     \eqn{\beta_{2}}.  Association parameters quantifying the link between
#'     the sub-models are represented by \eqn{\alpha} terms.
#'
...
#'
#'     #fit multi-study joint model with study level random effects and
...
#'
#'     #fit multi-study joint model without study level random effects with
#'     #unstratified baseline hazard
...
jointmeta1 <- function(data, long.formula, long.rand.ind, long.rand.stud = NULL,
                     sharingstrct = c("randprop", "randsep", "value", "slope",
                                      "valandslope"),
                     surv.formula,
                     gpt, lgpt, max.it, tol, study.name, strat = F,
                     longsep = F, survsep = F, bootrun = F, print.detail = F){
  ...
  if((sharingstrct%in%c("randprop", "randsep",
                        "value", "slope", "valandslope")) == FALSE){
    stop("Invalid sharing structure specified")
  }
  if(sharingstrct%in%c("slope", "valandslope")){
    if(missing(derivbytime)){
      stop("Please specify the derivbytime variable")
    }
  }
  if(sharingstrct != "randprop"){
  ...
  if(is.null(long.rand.stud) == F){
    if(study.name%in%long.rand.stud){
      if(which(long.rand.stud%in%study.name) != 1){
        long.rand.stud <- long.rand.stud[-which(long.rand.stud%in%study.name)]
  ...
  longdat2 <- merge(data$longitudinal, data$baseline,
                    by = id.name, sort = FALSE)
  long.frame <- model.frame(long.formula, data = longdat2,
                            na.action=na.pass)
  ...
  for(count in 1:length(long.rand.ind)){
    if(long.rand.ind[count] != "noint"){
      q <- q + 1
      if(length(which(grepl(long.rand.ind[count],
                            colnames(long.cov)) == TRUE)) == 0){
        if(grepl(".",long.rand.ind[count])) {
          ...
          for(i in 1:nrow(combs)){
            if(!(combs[i,1]==combs[i,2])){
              if(length(which(grepl(paste(temp[combs[i,1]], temp[combs[i,2]],
                                          sep="."), colnames(long.cov))) ==
                        TRUE) >0){
                present<-TRUE
  ...
  if(is.null(long.rand.stud) == FALSE){
    ...
    for(count in 1:length(long.rand.stud)){
      if(long.rand.stud[count] != study.name){
        r <- r + 1
        if(length(which(grepl(long.rand.stud[count],
                              colnames(long.cov)) == TRUE)) == 0){
            ...
            for(i in 1:nrow(combs)){
              if(!(combs[i,1]==combs[i,2])){
                if(length(which(grepl(paste(temp[combs[i,1]],
                                      ...
                                      colnames(long.cov))) == TRUE) >0){
                  present<-TRUE
                  long.rand.stud[count]<-paste(temp[combs[i,1]],
                                               temp[combs[i,2]], sep=".")
                }
              }
            }
  ...
  longdat <- cbind(data$longitudinal[[id.name]][rll],
                   long.frame[, 1][rll], data$longitudinal[[time.long]][rll],
                   longdat2[[study.name]][rll], long.cov[rll, ])
  longdat <- as.data.frame(longdat)
  missingids<-unique(data$longitudinal[[id.name]][!rll])
  names(longdat) <- c(id.name, names(long.frame)[1],
                      time.long, study.name, long.names)
  long.formula <- as.formula(paste(as.character(long.formula)[2],
                                   "~", paste(names(longdat)[5:ncol(longdat)],
                                              collapse = " + "), sep = ""))
  p1 <- length(5:ncol(longdat))
  notinteractionterms <- names(longdat[,
                                       5:ncol(longdat)])[!(
                                         grepl(":",
                                               names(longdat[,
                                                             5:ncol(longdat)]
  ...
  for(count in 1:length(long.rand.ind)){
    if(length(grep(paste("^",long.rand.ind[count],"$",sep=""),
      ...
      long.rand.ind[count] <- notinteractionterms[
        grep(paste("^",long.rand.ind[count],"$",sep=""),
             notinteractionterms)]
    } else if(length(grep(paste("^",long.rand.ind[count],"$",sep=""),
                          notinteractionterms)) == 0) {
      if(long.rand.ind[count] %in% colnames(data$baseline)) {
        if(class(data$baseline[,
                               which(colnames(data$baseline) ==
                                     long.rand.ind[count])]) == "factor") {
          formtemp<-as.formula(paste("~",
  ...
  if(is.null(long.rand.stud) == FALSE){
    for(count in 1:length(long.rand.stud)){
      if(long.rand.stud[count] != study.name) {
        if(length(grep(paste("^",long.rand.stud[count],"$",sep=""),
                       notinteractionterms)) > 0){
          long.rand.stud[count] <- notinteractionterms[
            grep(paste("^",long.rand.stud[count],"$",sep=""),
                 notinteractionterms)]
        } else if(length(grep(paste("^",long.rand.stud[count],"$",sep=""),
                              notinteractionterms)) == 0) {
  ...
  survdat <- cbind(data$survival[[id.name]][rss],
                   srv[rss, 1], srv[rss, 2],
                   data$baseline[[study.name]][rss], surv.cov)
  survdat <- as.data.frame(survdat)
  names(survdat) <- c(id.name, surv.formula[2][[1]][[2]],
                      surv.formula[2][[1]][[3]], study.name,
                      colnames(surv.cov))
  if (dim(survdat)[2] > 4) {
  ...
  survdat2 <- data.frame(data$survival[[id.name]][rss], srv[rss, 1],
                         srv[rss, 2], data$baseline[[study.name]][rss],
                         surv.frame[, -1])
  if(ncol(survdat)>4){
    surv.formula <- as.formula(paste(as.character(surv.formula)[2],
                                     "~", paste(names(survdat)[5:ncol(survdat)],
                                                collapse = " + "), sep = ""))
    names(survdat2) <- c(id.name, surv.formula[2][[1]][[2]],
                         surv.formula[2][[1]][[3]], study.name,
                         colnames(surv.frame)[2:ncol(surv.frame)])
  }else {
    surv.formula <- as.formula(paste(as.character(surv.formula)[2],
                                     "~ 1", sep = ""))
    names(survdat2) <- c(id.name, surv.formula[2][[1]][[2]],
  ...
  if(sharingstrct == "randprop"){
    ...
    jointfit <- EMalgRandprop(data = data,
                              longdat = longdat, survdat = survdat,
                              long.rand.ind = long.rand.ind,
                              long.rand.stud = long.rand.stud,
                              id.name = id.name, study.name = study.name,
                              gpt = gpt, max.it = max.it, tol = tol,
                              time.long = time.long,
                              surv.formula = surv.formula,
                              long.formula = long.formula,
                              long.formula.orig = long.formula.orig,
                              paraests = paraests, studies = studies,
                              p1 = p1, p2 = p2, strat = strat,
    ...
    random_ind <- lapply(1:numstudies, function(u){
      ...
      colnames(randtemp) <- paste("b2_", 0:(ncol(randtemp) - 1), sep = "")
    ...
    if(is.null(long.rand.stud) == FALSE){
      ...
      names(latent) <- c(paste("gamma_ind_", 0, sep = ""),
                         paste("gamma_stud_", 0, sep = ""))
      random_stud <- jointfit$random3
      colnames(random_stud) <- paste("b3_", 0:(ncol(random_stud) - 1), sep = "")
      ...
      if(study.name%in%long.rand.stud)
      {
        ...
        long.rand.stud.form <- paste("-1",
                                     paste(long.rand.stud, collapse = " + "),
                                     sep = " + ")
      }
    }else{
    ...
    if(bootrun == FALSE){
      ...
      jointll <- jlike(data = data, longdat = longdat, survdat = survdat, q = q,
                       likeests = likeests, lgpt = lgpt, studies = studies,
                       p1 = p1, p2 = p2, long.rand.ind = long.rand.ind,
                       randstart.ind = paraests$randstart.ind,
                       randstart.ind.cov = paraests$randstart.ind.cov,
                       r = r, long.rand.stud = long.rand.stud,
                       randstart.stud = randstart.stud.l,
                       randstart.stud.cov = randstart.stud.cov.l,
                       strat = strat, study.name = study.name,
                       id.name = id.name)
      numpara<- p1 + p2 + (q^2) + 2
      if(!is.null(long.rand.stud)) {
    ...
    formulae <- list(lformula = long.formula,
                    ...
                    rand_ind_formula = as.formula(paste("~",
                                                        long.rand.ind.form,
                                                        sep = "")))
    ...
    if(is.null(long.rand.stud) == FALSE){
      formulae$rand_stud_formula = as.formula(paste("~",
                                                    long.rand.stud.form,
                                                    sep = ""))
    ...
    results <- list(coefficients = coefficients, sigma.e = jointfit$sigma.e,
                    ...
                    sharingstrct = sharingstrct, sepests = sepests,
                    sep.loglik = sep.loglik, data=data, Call = Call,
                    random_cond = random_cond, numstudies = numstudies,
                    n.bystudy = n.bystudy, missingids = missingids,
                    nobs = nobs, AIC = AIC)
