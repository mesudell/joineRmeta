...
#'     vectors each of length equal to the total number of random
#'     effects. If the same association parameters are supplied for each study
#'     in the dataset then this is a vector of length equal to the number of
#'     random effects.  If separate association parameters are
#'     defined for different random effects (i.e. if \code{sepassoc = TRUE})
#'     then the elements in each of these vectors are not necessarily identical.
#'     If \code{sepassoc = FALSE} then the association parameters within each
#'     vector are identical.  In each vector of association parameters the first
#'     \code{q} values are the association parameters for the individual level
#'     random effects, and the remaining values are the association parameters
#'     for the study level random effects.
...
simdat2randlevels <- function(k, n, rand_ind, rand_stud, sepassoc, ntms,
                              longmeasuretimes, beta1, beta2, gamma, sigb_ind,
                              sigb_stud, vare, theta0, theta1, censoring,
                              censlam, truncation, trunctime, q, r) {
  treat <- lapply(1:k, function(u) {rbinom(n[u], 1, 0.5)})
  X2 <- lapply(1:k, function(u) {cbind(treat = treat[[u]])})
  ...
  time <- lapply(1:k, function(u) {rep(longmeasuretimes,
                                       length = n[[u]] * ntms)})
  X1 <- lapply(1:k, function(u) {cbind(intercept = 1, treat = treatl[[u]],
                                       ltime = time[[u]])})
  U_ind <- lapply(1:k, function(u) {mvrnorm(n[u], mu = rep(0, q),
                                            Sigma = sigb_ind)})
  U_stud <- mvrnorm(k, mu = rep(0, r), Sigma = sigb_stud)
  U_ind_l <- lapply(1:k, function(u) {U_ind[[u]][rep(1:n[u], each = ntms), ]})
  U_stud_l <- lapply(1:k, function(u) {U_stud[rep(u, each = (ntms * n[u])), ]})
  ...
  Z2b2 <- lapply(1:k, function(u) {t((Z2[[u]]) * as.vector(U_ind_l[[u]]))})
  Z3b3 <- lapply(1:k, function(u) {t((Z3[[u]]) * as.vector(U_stud_l[[u]]))})
  Y <- lapply(1:k, function(u) {
    X1[[u]] %*% beta1 + colSums(Z2b2[[u]]) +
      colSums(Z3b3[[u]]) + sqrt(vare) * rnorm(n[u] * ntms)
    })
  if(rand_stud == "inttreat") {
  ...
  beta2x <- lapply(1:k, function(u) {X2[[u]] %*% beta2})
  uu <- lapply(1:k, function(u) {runif(n[u])})
  if(rand_ind == "int") {
    if(class(gamma) == "list") {
      survtime <- lapply(1:k, function(u) {
        -log(uu[[u]]) / exp(theta0[[u]] +
                              beta2x[[u]] +
                              (U_ind[[u]] * gamma[[u]][1]) +
                              colSums(Z3b3.surv[[u]] *
                                        gamma[[u]][(q + 1):(q + r)]))
      ...
      survtime <- lapply(1:k, function(u) {
        -log(uu[[u]]) / exp(theta0[[u]] + beta2x[[u]] +
                              (U_ind[[u]] * gamma[1]) +
                              colSums(Z3b3.surv[[u]] * gamma[(q + 1):(q + r)]))
      })
    }
  }else{
    if(class(gamma) == "list") {
      survtime <- lapply(1:k, function(u) {
        ...
        temp2 <- exp(theta0[[u]] + beta2x[[u]] +
                       (gamma[[u]][1] * U_ind[[u]][, 1]) +
                       colSums(Z3b3.surv[[u]] * gamma[[u]][(q + 1):(q + r)]))
        ii <- (temp1<0)&(uu[[u]]<exp(temp2 / temp1))
        survtime <- rep(0, n[u])
        survtime[ii] <- Inf
        survtime[!ii] <- (1 / temp1[!ii]) *
          log(1 + (((temp1[!ii]) * (-log(uu[[u]][!ii]))) / temp2[!ii]))
        survtime
      })
    }else{
      survtime <- lapply(1:k, function(u) {
  ...
  if(censoring) {
    censtime <- lapply(1:k, function(u) {-log(runif(n[u])) / censlam[u]})
  }else{
    censtime <- lapply(1:k, function(u) {rep(Inf, n[u])})
  }
  if(truncation) {
    censtime <- lapply(1:k, function(u) {pmin(censtime[[u]], trunctime)})
  }
  ii <- lapply(1:k, function(u) {censtime[[u]]<survtime[[u]]})
  survtime <- lapply(1:k, function(u) {
    temp <- survtime[[u]]
  ...
  longdat <- lapply(1:k, function(u) {
    data.frame(id = idl[[u]], Y = Y[[u]], time = time[[u]],
               study = rep(u, length(idl[[u]])), X1[[u]])
  })
