...
#'     coefficients of the fixed effect parameters of the longitudinal
#'     sub-model.}
#'
...
#'     one vector.}
#'
#'     \item{\code{rs}}{a counter to indicate the last how many unique event
#'     times had occured by the individual's survival time - this is for use
#'     during further calculation in the joint model EM algorithm.  If a
...
#'
#'     }
#' @param randstart.ind a list of the conditional modes of the
...
#' @param randstart.ind.cov a list of the conditional covariance
#'     matrices for each individual for the individual level random effects
#'     given the data and the estimates of the separate longitudinal model
#'     parameters
#' @param r the number of study level random effects (if included in the model)
#' @param randstart.stud a data frame containing the conditional modes
...
#'     level random effects were specified in the \code{jointmeta1} function
#'     call.
#' @param randstart.stud.cov a list of conditional covariance matrices
...
#'
#' \item{\code{log.like}}{the overall log-likelihood for the fitted joint model.
#' }
#'
#' \item{\code{longlog.like}}{the portion of the log-likelihood attributable to
#' the longitudinal sub-model.}
#'
#' \item{\code{survlog.like}}{the portion of the log-likelihood attributable to
...
jlike <- function(data, longdat, survdat, q, likeests, lgpt, studies, p1, p2,
  ...
  ab <- g$nodes
  w <- g$weights * sqrt(pi)
  resid <- lapply(1:numstudies, function(u) {Y.bystudy[[u]] -
      X1.bystudy[[u]] %*% beta1})
  ...
  cnn.bystudy <- lapply(1:numstudies, function(u) {c(0, cumsum(nn[[u]]))})
  if (p2 == 0) {
    beta2x <- lapply(1:numstudies, function(u) {matrix(0, n.bystudy[[u]], 1)})
  }else{
    beta2x <- lapply(1:numstudies, function(u) {X2.bystudy[[u]] %*% beta2[1:p2]})
  }
  sigma.ei <- lapply(1:numstudies, function(u) {sigma.e * diag(max(nn[[u]]))})
  cov2 <- lapply(1:numstudies, function(u) {D %*% Z2.bystudy[[u]]})
  ...
  if(is.null(r)) {
    ...
    cov3 <- lapply(1:numstudies, function(u) {A %*% Z3.bystudy[[u]]})
    randstart.stud <- likeests$random3
    eig2 <- eigen(D)
    eig3 <- eigen(A)
    if(length(which(eig2$values < 0)) == 0 &&
       length(which(eig3$values < 0)) == 0) {
      if(q > 1) {
  ...
  for(k in 1:numstudies) {
    for (i in 1:n.bystudy[k]) {
      ...
      newu <- newu.all[[k]][[i]]
      if(q > 1) {
        tZ2.i <- tZ2.bystudy[[k]][(cnn.bystudy[[k]][i] + 1):
                                    cnn.bystudy[[k]][i + 1], ]
      }else{
        tZ2.i <- tZ2.bystudy[[k]][(cnn.bystudy[[k]][i] + 1):
                                    cnn.bystudy[[k]][i + 1]]
      }
      U21.2 <- cov2[[k]][, (cnn.bystudy[[k]][i] + 1):cnn.bystudy[[k]][i + 1]]
      Z3iU31 <- matrix(0, nrow = nn[[k]][[i]], ncol = nn[[k]][[i]])
      if(is.null(r) == FALSE) {
        U31.2 <- cov3[[k]][, (cnn.bystudy[[k]][i] + 1):cnn.bystudy[[k]][i + 1]]
        if(r > 1) {
          tZ3.i <- tZ3.bystudy[[k]][(cnn.bystudy[[k]][i] + 1):
                                      cnn.bystudy[[k]][i + 1], ]
          Z3iU31 <- tZ3.i %*% U31.2
        }else{
          tZ3.i <- tZ3.bystudy[[k]][(cnn.bystudy[[k]][i] + 1):
        ...
        if(r == 1) {
          expZ3u3 <- exp(beta2[p2 + 2] * (newu[, 1:r] *
                                            Z3.surv.bystudy[[k]][i]))
        }else{
          expZ3u3 <- exp(beta2[p2 + 2] * (newu[, 1:r] %*%
                                            Z3.surv.bystudy[[k]][, i]))
        ...
        expZu.event <- exp(beta2[p2 + 1] * (newu[, (r + 1):(r + q)] %*%
                                              Z2.event.bystudy[[k]][[i]])) *
          exp(beta2[p2 + 2] * (newu[, 1:r] %*% Z3.event.bystudy[[k]][[i]]))
      }else{
        if(q == 1) {
          expZu <- exp(beta2[p2 + 1] * (newu[, 1:q] * Z2.surv.bystudy[[k]][i]))
        }else{
          expZu <- exp(beta2[p2 + 1] * (newu[, 1:q] %*%
                                          Z2.surv.bystudy[[k]][, i]))
        }
        expZu.event <- exp(beta2[p2 + 1] * (newu[, 1:q] %*%
                                              Z2.event.bystudy[[k]][[i]]))
      }
      if(q == 1) {
        U11.2 <- tcrossprod(tZ2.i, U21.2) + Z3iU31 +
          sigma.ei[[k]][1:nn[[k]][i], 1:nn[[k]][i]]
      }else{
        U11.2 <- tZ2.i %*% U21.2 + Z3iU31 +
          sigma.ei[[k]][1:nn[[k]][i], 1:nn[[k]][i]]
      }
      if(rstemp > 0) {
        den <- sum(((haz.i * exp(beta2x[[k]][i, ]) *
                       expZu)^cen.bystudy[[k]][[i]]) *
                     (exp(-((exp(beta2x[[k]][i, ]) *
                               expZu.event) %*% haz.i.rs))) * w)
      }else{
        den <- sum((exp(-((exp(beta2x[[k]][i, ]) *
                             expZu.event) %*% matrix(0)))) * w)
      }
      resid.i <- resid[[k]][(cnn.bystudy[[k]][i] + 1):cnn.bystudy[[k]][i + 1]]
      l2 <- l2 + 0
      if (den > 0) {
        l2 <- l2 + log(den)
      }
