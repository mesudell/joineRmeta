...
#'     \code{\link{jointmeta1}} function to joint longitudinal and survival
#'     data.
...
#'     of the longitudinal sub-model, \code{"Survival"} will result in the
#'     formula for the fixed effect portion of the survival sub-model being
#'     returned.  To return the formula for the individual level random effect,
#'     specify \code{type = "Rand_ind"}, or to return the formula for the study
#'     level random effects (if included in the joint model), specify
#'     \code{type = "Rand_stud"}.
#'
...
formula.jointmeta1 <- function(fitted, type = c("Longitudinal", "Survival",
  ...
  if(missing(type) || !(type %in% c("Longitudinal", "Survival",
                                    "Rand_ind", "Rand_stud"))) {
  ...
  if(type == "Longitudinal") {
    ...
    out <- as.formula(paste("~", paste(randnames,
                                    collapse=" +"), sep=""), env=globalenv())
